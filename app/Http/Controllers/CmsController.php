...
class CmsController extends Controller
{
    ...
    public function updateSettings(Request $request)
    {
        $validated = $request->validate([
            ...
            'about_image_file' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg,webp|max:5120',
            'hero_layout' => 'nullable|string',
            'hero_image_size' => 'nullable|string',
        ]);

        if ($request->hasFile('hero_image_file')) {
            $image = $request->file('hero_image_file');
            $name = time() . '_hero_' . $image->getClientOriginalName();
            $destinationPath = public_path('/uploads');
            if (!File::exists($destinationPath)) {
                File::makeDirectory($destinationPath, 0755, true);
            }
            $image->move($destinationPath, $name);
            $validated['hero_image_url'] = '/uploads/' . $name;
        }

        if ($request->hasFile('about_image_file')) {
            $image = $request->file('about_image_file');
            $name = time() . '_about_' . $image->getClientOriginalName();
            $destinationPath = public_path('/uploads');
            if (!File::exists($destinationPath)) {
                File::makeDirectory($destinationPath, 0755, true);
            }
            $image->move($destinationPath, $name);
            $validated['about_image_url'] = '/uploads/' . $name;
        }

        unset($validated['hero_image_file']);
        unset($validated['about_image_file']);
